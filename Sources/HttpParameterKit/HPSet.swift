		...
		do {
			return try (parameters + optionals).compactMap { item in
				...
				case let nilItem as HPNil:
					return try nilItem.query(())
				case let boolItem as HPBool:
					return try boolItem.query(value[boolItem.name] as? Bool)
				case let stringItem as HPString:
					return try stringItem.query(value[stringItem.name] as? String)
				case let integerItem as HPInt:
					return try integerItem.query(value[integerItem.name] as? Int)
				case let int8Item as HPInt8:
					return try int8Item.query(value[int8Item.name] as? Int8)
				case let int16Item as HPInt16:
					return try int16Item.query(value[int16Item.name] as? Int16)
				case let int32Item as HPInt32:
					return try int32Item.query(value[int32Item.name] as? Int32)
				case let int64Item as HPInt64:
					return try int64Item.query(value[int64Item.name] as? Int64)
				case let uintItem as HPUInt:
					return try uintItem.query(value[uintItem.name] as? UInt)
				case let uint8Item as HPUInt8:
					return try uint8Item.query(value[uint8Item.name] as? UInt8)
				case let uint16Item as HPUInt16:
					return try uint16Item.query(value[uint16Item.name] as? UInt16)
